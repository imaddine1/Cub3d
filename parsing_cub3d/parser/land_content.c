...
/*   Updated: 2022/10/06 16:03:07 by iharile          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (divide_lines[++i])
	{
		...
		if (divide_lines[i][j] == '\0')
			free_and_msg(divide_lines, NULL, main_str,
				"\033[0;31mland is incorrect:check the spaces\033[0;m");
		k = j - 1;
		...
		norm_walls(divide_lines, i, j, main_str);
	}
...
char	land_numbers(char *s, char *main_str)
{
	...
	while (s[++i])
	{
		...
		if (s[i] == 'N' || s[i] == 'S' || s[i] == 'E' || s[i] == 'W')
		{
			...
			free_and_msg(NULL, NULL, main_str,
				"\033[0;3you put something not allowed\033[0;31m");
	}
